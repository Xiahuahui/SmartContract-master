        ...
        if c_stat == 1:       #当该承诺状态为1时
            ...
            if connect:                  #如果该承诺含有对其他承诺的依赖，则 需要检查依赖承诺是否满足 满足则置为2  否则跳过
                ...
                if s[con_id] == con_stat:
                    change = 2
                    (queue,transfers,Id,Repeat)=handel (st,i,change,'Ture-'+connect,queue,transfers,Id,Repeat)
                else:
                    change = 4         #当前体条件过期时则转变为4
                    (queue,transfers,Id,Repeat)=handel (st,i,change,'Timeout-'+connect,queue,transfers,Id,Repeat) 
                    #状态转移处理函数，对状态队列，状态转移队列进行更新
                    
            else:               #如果该承诺不含有对其他承诺的依赖，则置为2 
                change = 2
                (queue,transfers,Id,Repeat)=handel (st,i,change,'Ture-'+event,queue,transfers,Id,Repeat)
                #状态转移处理函数，对状态队列，状态转移队列进行更新
                change = 4         #当前体条件过期时则转变为4
                (queue,transfers,Id,Repeat)=handel (st,i,change,'Timeout-'+event,queue,transfers,Id,Repeat) 
                 #状态转移处理函数，对状态队列，状态转移队列进行更新
               
        elif c_stat == 2:
            change = 3      #该承诺的后果按时完成   则置为3
            (queue,transfers,Id,Repeat)=handel (st,i,change,'Ture-'+CC[i].res,queue,transfers,Id,Repeat) 
             #状态转移处理函数，对状态队列，状态转移队列进行更新
            change = 5      #该承诺的后果过期   则置为5
            (queue,transfers,Id,Repeat)=handel (st,i,change,'Timeout-'+CC[i].res,queue,transfers,Id,Repeat)  
            #状态转移处理函数，对状态队列，状态转移队列进行更新  
